...
('admin', '123', 'Alice Murphy', 'alice.murphy@university.edu', 'ADMIN', 0),
('tech', '123', 'Daniel Walsh', 'daniel.walsh@university.edu', 'TECHNICIAN', 0),
('student_ai', '123', 'Chen Wei', 'chen.wei@student.edu', 'STUDENT', 1),
('student_bio', '123', 'Emma Zhang', 'emma.zhang@student.edu', 'STUDENT', 0),
...
INSERT INTO reservations (requester_id, equipment_id, start_time, end_time, purpose, status) VALUES
(4, 3, DATE_ADD(CURDATE(), INTERVAL 2 DAY) + INTERVAL 8 HOUR, DATE_ADD(CURDATE(), INTERVAL 2 DAY) + INTERVAL 14 HOUR, 'Measure sensor output for lab exercise', 'APPROVED'),
(5, 5, DATE_ADD(CURDATE(), INTERVAL 3 DAY) + INTERVAL 14 HOUR, DATE_ADD(CURDATE(), INTERVAL 3 DAY) + INTERVAL 20 HOUR, 'IoT gateway demo preparation', 'PENDING'),
(6, 1, DATE_ADD(CURDATE(), INTERVAL 4 DAY) + INTERVAL 20 HOUR, DATE_ADD(CURDATE(), INTERVAL 5 DAY) + INTERVAL 8 HOUR, 'Train small image classifier', 'APPROVED');

INSERT INTO approvals (reservation_id, approver_id, decision, comment) VALUES
(1, 1, 'APPROVED', 'Approved for scheduled lab work'),
(3, 1, 'APPROVED', 'Use the workstation for two hours only');

INSERT INTO maintenance_tickets (equipment_id, reporter_id, technician_id, title, description, priority, status) VALUES
(4, 4, 2, 'ECG cable is loose', 'The signal drops when the sensor cable is moved.', 'HIGH', 'IN_PROGRESS'),
(6, 5, NULL, 'Switch fan noise', 'The switch fan is louder than usual during long lab sessions.', 'MEDIUM', 'OPEN');

INSERT INTO maintenance_updates (ticket_id, user_id, update_text) VALUES
(1, 2, 'Checked the cable connector. Need to order a replacement cable.'),
(2, 5, 'Reported after network lab practice.');
