...
I implemented and tested the reservation workflow, including request creation and approval handling. I paid special attention to the overlapping time condition because it is an important database logic problem. I also helped integrate the UI tabs and prepared run instructions.

### Member B

I finalised the database schema and seed data. I added constraints, indexes, and views to support the main system functions. I also checked that the ER diagram matches the SQL schema and prepared explanations for the relational mapping.

### Member C

I implemented the equipment and maintenance workflow. I connected maintenance ticket creation with equipment status updates, so faulty equipment cannot continue to be booked. I also prepared the maintenance part of the demonstration.

### Member D

I implemented the inventory and report modules. The inventory module records stock transactions and uses transaction logic to prevent invalid stock values. I also created report queries for lab usage and equipment status summaries.

### Member E

I worked on UI consistency, model classes, shared UI helpers, and testing documentation. I wrote automated tests for login, reservation conflict, inventory changes, and maintenance status changes. I also prepared manual test cases and the video script draft.
